...
void 	procClientRequest(void 	*arg)
{
		...
		while(1)
		{
			...
			memset(threadSno, 0, sizeof(threadSno));
			snprintf(threadSno, sizeof(threadSno), "thread %d:", pthread_self());
			nread = read(cFd, msgBuf, sizeof(msgBuf)-1);
			/* read failed */
			if(nread < 0)
			{
			...
			else
			{
				...
				write(cFd, threadSno, strlen(threadSno));
				write(cFd, msgBuf, strlen(msgBuf));
			}
		}
}
...
/***********************************************
    ...
    Return       :   ERROR_SUCCESS   成功
					 ERROR_FAILED    失败
************************************************/
ulong 	addRequest2JobQueue(ThreadPools 	*pThreadPool, THREAD_CALLBACK_PF 	pfThreadCallback, void 	*arg)
{
